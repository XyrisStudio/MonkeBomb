...
void FAnimNode_DragonSpineSolver::Snake_ImpactRotation(int origin_point_index, FTransform& OutputTransform, FCSPose<FCompactPose>& MeshBases)
{
	...
	FVector new_location = owner_skel_w_transform.InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Spine_Point + (character_direction_vector_CS * Total_spine_heights[origin_point_index]));
	...
	else if (origin_point_index == spine_Transform_pairs.Num() - 1)
	{
		...
		if (owning_skel->GetOwner())
		{
			...
			right_direction = UKismetMathLibrary::TransformDirection(owner_skel_w_transform, FVector::CrossProduct(character_direction_vector_CS, Forward_Direction_Vector)) * right_intensity;
...
DragonSpineOutput FAnimNode_DragonSpineSolver::DragonSpineProcessor(FTransform& EffectorTransform, FCSPose<FCompactPose>& MeshBases, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	FVector lerp_data = owner_skel_w_transform.TransformPosition(RootTraceTransform.GetLocation());

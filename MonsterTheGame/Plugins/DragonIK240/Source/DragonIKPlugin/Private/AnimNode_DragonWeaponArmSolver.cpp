	...
	if(turn_on_solver)
	{
		...
		if (weapon_handler_component != NULL && weapon_handler_component != nullptr)
		{
			...
			for (int alpha_index = 0; alpha_index < grab_alpha.Num(); alpha_index++)
			{
				...
				if(weapon_handler_component->arm_reset_queue_index.Num() > 0)
				...
				if(Hand_Array.Num() == grab_alpha.Num())
				{
					...
					if (weapon_handler_component->handbone_array_boneIndex.Num() > 0 && alpha_index < weapon_handler_component->handbone_array_boneIndex.Num())
		...
		//if (Use_Separate_Targets)
		{
			...
			{			
				...
				for (int arm_index = 0; arm_index < Arm_TargetLocation_Overrides.Arm_TargetLocation_Overrides.Num(); arm_index++)
				{
					...
					FTransform Arm_Transform_Modified = Arm_TargetLocation_Overrides.Arm_TargetLocation_Overrides[arm_index].Overrided_Arm_Transform;
					...
					{
						Arm_Position = Arm_Transform_Modified.GetLocation();
					}
					else
					{
		...
		for (int arm_index = 0; arm_index < Arm_TargetLocation_Overrides.Arm_TargetLocation_Overrides.Num(); arm_index++)
		{
			...
			if ((owning_skel->GetWorld()->IsGameWorld()))
			{
				...
				if (Debug_Hand_Locations.Num() > arm_index && weapon_handler_component != nullptr)
				{
					...
					bool should_override_hand_loc = false;
					bool should_override_hand_rot = false;



					if (weapon_handler_component->should_override_hand_rotation.Num() > 0 && weapon_handler_component->should_override_hand_location.Num() > 0)
					{
						should_override_hand_loc = weapon_handler_component->should_override_hand_location[arm_index];
						should_override_hand_rot = weapon_handler_component->should_override_hand_rotation[arm_index];
					}
